...
function _sendRequest($url, $key, $data = [], $method = 'GET', $isRapidAPI = true, $rapidAPIHost = "")
{
    ...
    $options = [
        ...
        CURLOPT_CUSTOMREQUEST => ($method == 'GET2')? "GET":$method,
    ...
    if ($method == 'GET') {
        ...
        $other_params = http_build_query(array_slice($data,1));
        $options [CURLOPT_URL] = "$url/" . $data[0] . (empty($other_params)?'':'?' . $other_params);
    }
    else {
...
/**
 * Send a GET request to the specified URL but modified to not have question mark and key name before gameID.
 * 
 * @param string $url The URL to send the request to.
 * @param string $key The API key to use for the request.
 * @param array $data The data to send with the request.
 ...
 * @return array The response status and body.
 */
function get2($url, $key, $data = [], $isRapidAPI = true,  $rapidAPIHost = "")
{
    return _sendRequest($url, $key, $data, 'GET2', $isRapidAPI, $rapidAPIHost);
}
